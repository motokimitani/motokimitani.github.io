...
//【プレイヤー4】
var diceSum4 = 0; //出目の合計（現在のマス）
var diceSumId4 = ("m" + diceSum4); //出目の合計ID（現在のマスID）
var leftGoal4 = 100; //残りのマス数
var countNum4 = 0; //サイコロを投げた回数
...
//サイコロを振った時の表示処理【プレイヤー4】
function start_function4() {
$(`#resetDice`).prop(`disabled`,true); //はじめからボタンは押せない
diceNum = Math.floor(Math.random() * 6) + 1; //1～6のランダムな出目を生成
$(`img`).attr(`src`, `${diceNum}.png`);
$(`#dice-deme`).text(diceNum); //出目を表示
countNum4++; //サイコロを投げた回数が加算
$(`#count-dice`).text(countNum4 + "投目"); //サイコロを投げた回数を表示
leftGoal4 = leftGoal4 - diceNum;
diceSum4 = 100 - leftGoal4;
$(`#current-place`).text("現在"+diceSum4+"マス目"); //現在マス位置の表示
$(`#left-goal`).text("ゴールまであと"+leftGoal4+"マス"); //残りマスの表示
$(`#event-stop`).html(``); //イベント発生コメントは非表示
}
...
//駒を進める処理【プレイヤー4】
function forward_function4 () {
if (diceSum4 < 100 ) { //100マス未満の場合
player4 = document.getElementById(`currentPosition4`);
document.getElementById(diceSumId4).removeChild(player4);
diceSumId4 = ("m" + diceSum4); //出目の合計ID（現在のマス）
document.getElementById(diceSumId4).insertAdjacentHTML('beforeend','<i id="currentPosition4" class="fas fa-biking play4"></i>');
element = document.getElementById(`currentPosition4`); //要素(駒)の現在地を取得
element.scrollIntoView({behavior: 'smooth',block: 'center'});//スクロール時の要素(駒)の位置指定
} else {
//ゴール時の駒の処理
document.getElementById(diceSumId4).innerHTML = "";
diceSum4 = 100;
diceSumId4 = ("m" + diceSum4); 
document.getElementById(diceSumId4).insertAdjacentHTML('beforeend','<i id="currentPosition4" class="fas fa-child play4"></i>');
element = document.getElementById(`currentPosition4`);
element.scrollIntoView({behavior: 'smooth',block: 'center'});
$(`#resetDice`).prop(`disabled`,false); //ゴール時ははじめからボタンを押せる
}
} 
...
//あたりイベントorはずれイベントをランダムで選択【プレイヤー4】
function select_event4(){
if (document.getElementById(diceSumId4).classList.contains("event") == true) { //"event"クラスの存在を確認
  var eventSelect = Math.floor(Math.random()* 2) + 1; 
if (eventSelect == 1){ //あたりイベントの場合
  setTimeout("forward_event4()",950); //イベント発生までの待機時間
} else if(eventSelect == 2) { //はずれイベントの場合
  setTimeout("back_event4()",950); //イベント発生までの待機時間
}
}
}
...
//イベントマスで進む処理（あたりイベント）【プレイヤー4】
function forward_event4(){
eventCount = Math.floor(Math.random()* 3) + 1; //1～3の数字をランダムで生成
$(`#event-stop`).text("あたりイベント！"+eventCount+"マス進む"); //あたりイベント+進むマス数の表示
document.getElementById(diceSumId4).innerHTML = "";
diceSum4 = diceSum4 + eventCount; //イベント発生時の駒の動き
diceSumId4 = ("m" + diceSum4); 
leftGoal4 = leftGoal4 - eventCount; //残りマスを確認
document.getElementById(diceSumId4).insertAdjacentHTML('beforeend','<i id="currentPosition4" class="fas fa-biking play4"></i>');
element = document.getElementById(`currentPosition4`);
element.scrollIntoView({behavior: 'smooth',block: 'center'});
}
...
//イベントマスで戻る処理（はずれイベント）【プレイヤー4】
function back_event4(){
eventCount = Math.floor(Math.random()* 3) + 1; //1～3の数字をランダムで生成
$(`#event-stop`).text("はずれイベント！"+eventCount+"マス戻る");//はずれイベント+戻るマス数の表示
document.getElementById(diceSumId4).innerHTML = "";
diceSum4 = diceSum4 - eventCount; //イベント発生時の駒の動き
...
element = document.getElementById(`currentPosition4`);
element.scrollIntoView({behavior: 'smooth',block: 'center'});
$(`#count-dice`).text(countNum4+1 + "投目");
$(`#current-place`).text("現在"+diceSum4+"マス目"); 
$(`#left-goal`).text("ゴールまであと" + leftGoal4 + "マス");
$(`#player-turn`).html("プレイヤー4の順番です").css('color','blue');
$(`#event-stop`).text("");
}

function player4_turn(){
document.getElementById(`startDice4`).onclick = player3_action;
element = document.getElementById(`currentPosition1`);
element.scrollIntoView({behavior: 'smooth',block: 'center'});
...
if(leftGoal1 <= 0) { //ゴールした場合
  ...
  $(`#startDice4`).prop(`disabled`,true); //サイコロをふるボタンは押せない
  }
}
...
$(`#startDice2`).prop(`disabled`,true); 
$(`#startDice3`).prop(`disabled`,true); 
$(`#startDice4`).prop(`disabled`,false); 
...
function player4_action() {
document.getElementById(`startDice4`).onclick = "";
element = document.getElementById(`currentPosition4`);
element.scrollIntoView({behavior: 'smooth',block: 'center'});
...
$(`#startDice3`).prop(`disabled`,true); 
$(`#startDice4`).prop(`disabled`,true); 
var timeId = setTimeout("player4_turn()",2000);
if(leftGoal4 <= 0) { //ゴールした場合
  clearTimeout(timeId);
  $(`#left-goal`).text("ゴール!!プレイヤー4の勝利です!!").css("color","blue"); //ゴール(残りマス0)した時の表示
  $(`#startDice1`).prop(`disabled`,true); //サイコロをふるボタンは押せない
  $(`#startDice2`).prop(`disabled`,true); //サイコロをふるボタンは押せない
  $(`#startDice3`).prop(`disabled`,true); //サイコロをふるボタンは押せない
  $(`#startDice4`).prop(`disabled`,true); //サイコロをふるボタンは押せない
...
$(function(){
  ...
  $(`#startDice4`).click(start_function4);
  $(`#startDice4`).click(forward_function4);
  $(`#startDice4`).click(select_event4);
  $(`#startDice4`).click(player4_action);
